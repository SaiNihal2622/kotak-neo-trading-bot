...
msg_lines += [
    f"",
    f"Signals: {sig_total} (regime breakdown: {dict(sig_regimes)})",
    f"Paper account: Cash Rs.{paper['cash']:,.2f} | Cum P&L Rs.{paper['realized_pnl']:+,.2f}",
]

# Flag the square-off bug if present
if exit_reasons.get("eod_square_off", 0) and all(t.get("closed_at", "").startswith(TODAY + "T09:45") for t in trades_today):
    msg_lines += [
        f"",
        f"\u26A0\uFE0F Both trades were force-closed at 09:45 with exit_reason='eod_square_off' (0 P&L). "
        f"Square-off is firing ~6h early — clock or eod_square_off config bug. Worth checking.",
    ]

# Tomorrow outlook based on regime
if sig_regimes:
    ...
    if dominant == "range":
        ...
        outlook = "trending; prefer directional debit spreads or skip into expiry-day"
