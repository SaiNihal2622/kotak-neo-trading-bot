...
def render_recent_decisions(decisions):
    if not decisions:
        return '<section class="card"><h2>Recent LLM Decisions</h2><p class="muted">No LLM decisions yet today (waiting for events).</p></section>'
    rows = []
    ...
    return f'''
    <section class="card"><h2>Recent LLM Decisions</h2>
    <table class="data-table">
...
def render_bot_activity(bot_log_tail):
    """Recent orders + trades from bot log."""
    orders = []
    trades = []
    ...
    if trades:
        ...
        html.append('</pre></div>')
    if not html:
        html.append('<p class="muted">No recent orders/trades detected in bot log.</p>')
    return f'<section class="card"><h2>Bot Activity (recent)</h2>{"".join(html)}</section>'
...
def main() -> int:
    ...
    html = HTML_TEMPLATE.format(
        ...
        decisions=render_recent_decisions(decisions),
